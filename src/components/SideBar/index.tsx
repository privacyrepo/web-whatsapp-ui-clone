...
export default function SideBar() {
  const jobsList = jobs.job_list;
  const [search, setSearch] = useState("");
  const filteredJobsList =
    search.length > 0
      ? jobsList.filter((jobList) =>
          jobList.title.toLowerCase().includes(search)
        )
      : jobsList;
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col w-full overflow-y-scroll" id="job">
        {filteredJobsList.map((job, index) => {
          return <JobList key={index} isFirstJob={index == 0} data={job} />;
        })}
      </div>
    </div>
  );
